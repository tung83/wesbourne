...
class block extends base{
    function __construct($db,$lang){        
        parent::__construct($db,6,'block',$lang);
    }
    function ind_block(){ 
        $str.='<div class="row">
                    <div class="col-xs-12">
                        <div class="title-head">
                            <span>'.$this->title.'
                                ...
                                .common::qtext($this->db,$this->lang,10).
                        '</div>                   
                    </div>                  
                </div>'; 
        return $str;
    ...
    function block_list_item($item,$type=1){
        $lnk=myWeb.$this->view.'/'.common::slug($item['title']).'-i'.$item['id'];
        $img=$this->first_image($item['id']);
        if(trim($img)==='') $img='holder.js/400x300';else $img=webPath.$img;
        if($type==1){
            $str='
            <div class="col-md-12 col-sm-6 col-md-3 block-item">';    
        }else{
            $str='
            <div class="col-md-12 col-sm-6 col-md-4 block-item">';
        }        
        $str.='
        <a href="'.$lnk.'">
            <div>
    ...
    function category(){
        ...
        $this->db_orderBy();
        $list=$this->db->get('block_cate',null,'id,title');
        $str='
    ...
    function block_cate(){
        ...
        $this->db_orderBy();
        $this->db->pageLimit=6;
        $list=$this->db->paginate('block',$page);
        $count=$this->db->totalCount;
        $str.='<div class="block-list"> '
                . '<div class="container">'
                    . '<div class="row">';
        $str.=$this->ind_block($sum_text);
        if($count>0){
        ...
        if($pId==0){
            ...
            $cate=$this->db->where('id',$pId)->getOne('block_cate','id,title');       
    ...
    function block_one($id){
        $this->db->where('id',$id);
        $item=$this->db->getOne('block','id,price,price_reduce,title,content,pId,feature,manual,promotion,video');
        $this->db->where('pId',$item['pId'])->where('id',$item['id'],'<>')->where('active',1)->orderBy('rand()');
        $list=$this->db->get('block');
        $lnk=domain.'/'.$this->view.'/'.common::slug($item['title']).'-i'.$item['id'];
        $str.='
        <div class="row block-detail clearfix">
            
            <div class="col-xs-12">
            <div class="col-md-5">
                '.$this->block_image_show($item['id']).'
            </div>
        ...
        if(count($list)>0){
            ...
            $str.='<div class="slick block_list clearfix">';
    ...
    function first_image($id){
        ...
        $this->db_orderBy();
        $img=$this->db->getOne('block_image','img');
        return $img['img'];
    }
    function block_image_show($id){
        $this->db->reset();
        $this->db->where('active',1)->where('pId',$id);
        $this->db_orderBy();
        $list=$this->db->get('block_image');
        $temp=$tmp='';
        foreach($list as $item){
            $temp.='
            <li>
...
//	});  
    function block_cate_left_list(){
        ...
        $str.='
            <div class="col-md-3 block-col">
                <div class="block-item block-menu-container item">    
                    <ul class="block-menu">';
    ...
    function block_cate_list(){
        $this->db->reset();
        $this->db->where('active',1);
        $this->db_orderBy();
        $list=$this->db->get($this->db_cate_name);
        $str.='<ul class="block-menu">';
        foreach($list as $cate){
            $title=$cate['title'];
            $str.='
